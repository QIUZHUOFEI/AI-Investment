...
| 层级 | 环节 | 代表公司 |
|:----:|:-----|:---------|
| 🔵 **第一层** | **算力芯片** — GPU / 定制芯片 / 国产芯片 | 英伟达（NVDA）、Broadcom（AVGO）、AMD（AMD） |
| 📡 **第二层** | 硬件 & 元器件 — 光通信、存储芯片、PCB | 中际旭创（300308）、沪电股份（002463） |
| 🌡️ **第三层** | 基础设施配套 — 液冷、服务器、数据中心 | 英维克（002837）、浪潮信息（000977） |
| 🔬 **第四层** | 封测 & 制造 | 台积电（TSM）CoWoS、日月光（ASX） |
...
**CPU** 负责调度、加载数据、跑操作系统、控制整个流程；**GPU** 只管一件事——矩阵乘法。

> **AI 训练服务器的典型架构：** GPU × 8（主力算力）→ CPU（调度+数据搬运）→ HBM（GPU 的超高速内存）→ SSD（海量训练数据存储）
...
CUDA 早在 **2006 年推出**，比 AI 热潮早了将近 20 年。全世界有 **~500 万 CUDA 开发者**，所有主流 AI 框架（PyTorch、TensorFlow、JAX）都深度绑定 CUDA——它不只是一个编程语言，而是一套**完整的软件栈**，从底层驱动到上层模型库全被英伟达控制。换成 AMD 的 ROCm 或 Intel 的 oneAPI？**等于把所有代码重写一遍**，大部分公司根本不会这么做。
...
| 客户 | 芯片名称 | 代工伙伴 | 进展 |
|:-----|:---------|:---------|:-----|
| **Google（GOOGL）** | TPU（v7 Ironwood） | **Broadcom（AVGO）** | 合作超 10 年，已到第 7 代 |
| **Meta（META）** | MTIA | **Broadcom（AVGO）** | 协议到 2029 |
| **OpenAI（-）** | 首款定制 XPU | **Broadcom（AVGO）** | 2026 年加入，预计 2027 出货 |
| **Anthropic（-）** | — | **Broadcom（AVGO）** | 最新加入 |
| **Amazon（AMZN）** | Trainium / Inferentia | **Marvell（MRVL）** | AWS 自研 |
...
| 公司 | 产品 | 定位 | 2026 年状态 |
|:-----|:-----|:-----|:------------|
| **寒武纪（688256）** | 思元系列 | AI 训练 + 推理芯片 | 国产 AI 芯片龙头，产品迭代中 |
| **海光信息（688041）** | 深算系列（DCU） | 类 GPU 架构，兼容 CUDA | 走兼容路线，降低开发者迁移成本 |
...
| 维度 | 传统架构（Scale-out） | 超节点架构（Scale-up） |
|:-----|:--------------------|:---------------------|
| **互联方式** | GPU → 交换机 → GPU（间接互联） | GPU ←→ GPU（光纤直连） |
| **互联密度** | 基准 | **10 倍+** |
| **适用场景** | 通用分布式训练 | 国产芯片弥补单卡差距 |

**超节点（Scale-up）的核心逻辑**：

其核心是用**更密集的互联**（光纤直连/交换芯片）把更多 GPU 绑成一台"超级计算机"——全国产芯片集群，用 **数量换质量**，单卡性能只有英伟达的 50%，但用 2 倍的卡通过密集互联实现接近的性能。这个逻辑直接利好了国产交换芯片（盛科通信）、光模块（中际旭创等）、PCB（深南电路）。
...
英伟达拥有 **~500 万 CUDA 开发者**，每年 GTC 大会的开发者都在增长。它提供了**完整工具链**：cuDNN（深度学习库）→ TensorRT（推理优化）→ Triton（推理服务器）→ Megatron（大模型训练框架）。所有主流框架都深度绑定——PyTorch 底层默认调用 CUDA，换成 AMD 的 ROCm？**相当于让所有开发者重学一套工具**。这就是极强的**网络效应**：用的人越多→优化的库越多→跑的应用越多→用的人更多。
...
| 方案 | 配置 | 单卡算力 | 系统性能 |
|:-----|:-----|:---------|:---------|
| **英伟达方案（基准）** | 1×B200 + HBM3E + NVLink + CUDA | **100%** | 完整 AI 训练能力 |
| **国产方案（超节点）** | 2×国产芯片 + 互联芯片 + 光纤直连 | **~50%** | 通过 2-3 倍数量达到 ~80% 性能 |

**这个逻辑的受益方**：

| 环节 | 受益公司 | 逻辑 |
|:-----|:---------|:-----|
| **交换芯片** | 盛科通信（688702） | 超节点需要更多交换机芯片 |
| **光模块/光纤** | 中际旭创（300308）、光通信全链 | 密集互联需要更多光纤连接 |
| **PCB/连接器** | 沪电股份（002463）、深南电路（002916） | 每多一张卡就需要配套 PCB |
| **液冷** | 英维克（002837）等 | 堆更多卡→功耗更高→液冷需求更大 |
| **服务器整机** | 浪潮信息（000977）、中科曙光（603019） | 国产 GPU 服务器出货量增加 |
...
美国对华芯片制裁层层加码，H100/B200/Rubin 都不能卖给中国。国产芯片的制程工艺受限于台积电断供（7nm 以上）或国内产能不足。一旦美国进一步收紧出口管制（如 HBM 限制），国产 AI 芯片的"超节点"逻辑也会受限。
...
Google、Meta、Amazon、OpenAI 都在搞自己的 XPU。**AI ASIC 市场 2026 年预计 $300-400 亿**，占 AI 芯片总市场 ~20%。随着大模型数量减少、头部模型定型，定制芯片的"固定场景优化"优势会更明显。英伟达通用 GPU 的"万能"优势，在单一场景下不如定制芯片的"专精"。**长期来看，定制芯片可能吃掉通用 GPU 在推理侧的份额**（训练侧英伟达仍占主导）。
...
| 方向 | 代表公司 | 核心逻辑 | 一句话判断 |
|:-----|:---------|:---------|:-----------|
| **通用 GPU** | 英伟达（NVDA，~80%）、AMD（AMD） | CUDA 生态无法复制，产品迭代最快 | ✅ 行业绝对龙头，最确定 |
| **定制 AI 芯片（XPU）** | **Broadcom（AVGO）**、Marvell（MRVL） | 大客户自研趋势下，定制芯片需求爆发 | ✅ 高增长，但客户集中度高 |
| **国产 AI 芯片** | 寒武纪（688256）、海光信息（688041） | 超节点集群化，政策驱动国产替代 | ⚠️ 弹性大但技术差距仍需弥合 |
